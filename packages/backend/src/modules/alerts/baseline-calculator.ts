...
export class BaselineCalculatorService {
  ...
  /**
   * Get current hourly log rate (last full hour from aggregate + partial current hour from raw logs)
   */
  async getCurrentHourlyRate(
    ...
    // Query last hour from raw logs for most recent data
  ...
  private async percentileP95(
    ...
    // Get hourly totals (sum across services/levels per bucket)
    let query = db
      .selectFrom('logs_hourly_stats')
      .select([
        'bucket',
    ...
    const counts = results.map(r => Number(r.hourly_total)).sort((a, b) => a - b);
  ...
  /**
   * Query logs_hourly_stats aggregate for a time range and sum the counts
   */
